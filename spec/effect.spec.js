...
define(['resemblejs'], function (resemble) {
  ...
  describe('Effects', function () {
    ...
    // class AnalyserEffect extends vd.effect.Audio {
    //   attach (movie) {
    //     super.attach(movie)
    //
    //     this.analyserNode = movie.actx.createAnalyser()
    ...
    //     super.detach()
    //
    //     this.inputNode.disconnect(this.analyserNode)
    //     this.analyserNode.disconnect(this.outputNode)
    //   }
    //
    //   getTimeData() {
    //
    ...
    describe('Audio', function () {
      const effects = [
        new vd.effect.Volume({ volume: 2 }),
        new vd.effect.Panner({ pan: -1 })
      ]
      ...
      effects.forEach(effect => {
        describe(effect.constructor.name, function () {
          it('should apply effect properly', async function () {
            ...
            const actual = await new vd.Movie({ canvas: dummyCanvas })
              .addLayer(
                new vd.layer.Audio({ startTime: 0, source })
                  .addEffect(new vd.effect.Volume({ volume: 2 }))
              )
