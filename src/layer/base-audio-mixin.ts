...
export function BaseAudioMixin<OptionsSuperclass extends BaseOptions> (superclass: Constructor<Base>): Constructor<BaseAudio> {
  ...
  class MixedBaseAudio extends superclass {
    private __audioNode: IAudioNode<AudioContext>
    ...
    protected get _audioNode (): IAudioNode<AudioContext> {
      return this.__audioNode
    }

    protected set _audioNode (node: IAudioNode<AudioContext>) {
      ...
      if (node && node !== prevNode) {
        ...
        node.connect = <T extends IAudioDestinationNode<AudioContext>>(destination: T, outputIndex?: number, inputIndex?: number): AudioNode => {
        ...
        node.disconnect = <T extends IAudioDestinationNode<AudioContext>>(destination?: T | number, output?: number, input?: number): AudioNode => {
